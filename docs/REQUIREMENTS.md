...
| REQ-004 | Guest reservation flow: salon → service → stylist (or any) → free slot → name + phone/email → confirmation | Booking persists with status `confirmed`; server rejects stale/invalid slot submissions (409/422) even when client is bypassed | planned |
| REQ-005 | Slot engine: free slots = working hours − bookings − service buffers; Europe/Helsinki, DST-correct | Unit tests cover: DST transition days (March/Oct), buffer application, booked-interval exclusion, multi-stylist calendars | planned |
| REQ-006 | Gap-fragmentation rule: hard block any booking that would leave a residual gap fragment < threshold; threshold is per-salon setting, default 45 min | Test: 3 h gap, mid-gap booking leaving < 45 min fragment → rejected 422; changing threshold changes outcome; admin manual bookings bypass (A1, approved) | planned |
| REQ-007 | Concurrency safety: no double-booking, enforced at DB level | Test: 20 parallel requests for the same slot → exactly 1 succeeds; no orphaned rows | planned |
| REQ-008 | Employee calendar UI: day/week views, create/edit/cancel bookings, visual gap indicators | Owner sees all salon bookings; employee sees only own; cancel frees the slot immediately | planned |
...
| REQ-011 | Email notifications: booking confirmation + cancellation confirmation via SMTP (A2, approved) | With valid SMTP env config, confirmation sends on booking; failed send is logged and does not fail the booking; secrets only via env vars, `.env.example` documents them | planned |
